...
export default {
  name: 'login',
  created() {
    const id = Number(this.$router.history.current.query.id)
    this.tplId = id
    this.initForEdit()
  },
  data() {
  ...
  methods: {
    async handleTemplateSave() {
      await contract.editTpl(this.tplId, { template_name: this.templateName, template_content: this.content })
      this.$router.back()
    },
    async initForEdit() {
      const res = await contract.getTemplateById(this.tplId)
      this.content = res.template_content
